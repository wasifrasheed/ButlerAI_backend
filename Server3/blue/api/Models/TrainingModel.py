...
def Model_Train(DIR_NAME,categories,nodes_req):
    ...
    if nodes_req > 2:
        ...
        model.add(Dense(512, activation='relu'))
        model.add(Dropout(0.3))
        model.add(Dense(nodes_req, activation='softmax'))
        ...
        print("---------------------")
        y = to_categorical(y, nodes_req)
        history = model.fit(X,y,epochs=30,validation_split=0.2,callbacks=callback)
        print(history)
        model_name = "model.h5"
        ...
        model.add(Dense(512, activation='relu'))
        model.add(Dropout(0.3))
        model.add(Dense(nodes_req-1, activation='sigmoid'))
        ...
        print("---------------------")
        history = model.fit(X,np.array(y),epochs=30,validation_split=0.2,callbacks=callback)
        print(history)
        model_name = "model.h5"
        model_path = DIR_NAME + "/" + model_name
        model.save(model_path, include_optimizer=False)
