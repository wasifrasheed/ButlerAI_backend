...
class Create_Project_Multiclass(Resource):
    def post(self):
        try:
            proj_path = dirname + "/Projects"
            ...
            proj_name = postedData['project_name']
            #proj_count = postedData['proj_count']
            project_class_list = postedData['project_class_list']
            print(len(project_class_list))
            proj_count = len(project_class_list)
            temp_index = []
            if proj_count >= 2:
                ...
                if not os.path.exists(name_and_path):
                    ...
                    for index in range(len(project_class_list)):

                        category = project_class_list[index]
                        path_category = name_and_path + "/" + category

                        if not os.path.exists(path_category):

                            
                            os.mkdir(path_category)
                            print("created category : ", path_category)
                            index+=1
                            print(index)

                        else:
                            shutil.rmtree(name_and_path)
                            print(proj_name, " folder already exists.")
                            ret={"status":401,"message":"category with this name already exist"}   
                            return jsonify(ret)
                ...
                print("Can't make projects which have ccategories less than 2.")
                ret={"status":401,"message":"Can't make projects which have ccategories less than 2."}         
...
#dog
#"https://cdn.pixabay.com/photo/2019/04/29/22/36/labrador-4167269_960_720.jpg",
#"https://cdn.pixabay.com/photo/2016/02/19/15/46/dog-1210559__340.jpg"

#fox
#"https://cdn.pixabay.com/photo/2019/12/04/08/57/animal-4672170_960_720.jpg",
...
class Train_Models(Resource):
    def post(self):
        try:
            ...
            categories = project_class_list
            nodes_req = len(categories)
            DIR_NAME = proj_path + "/" + proj_name

            retJson = Model_Train(DIR_NAME,categories,nodes_req)
...
class Prediction(Resource):
    def post(self):
        try:
            ...
            project_class_list = postedData['project_class_list']
            print(len(project_class_list))
            no_of_classes = len(project_class_list)
            ...
            retJson = Prediction_Func(url,categories,model,no_of_classes)
...
#Create_Project_Multiclass
api.add_resource(Create_Project_Multiclass, "/create_multiclass_project")
